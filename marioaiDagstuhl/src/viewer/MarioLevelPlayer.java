...
public class MarioLevelPlayer {
	...
	public static void main(String[] args) throws IOException {
		...
		//level = eval.levelFromLatentVector(randomGaussianDoubleArray(32));
		double[] vector = JsonToDoubleArray(strLatentVector);
		String levelAI = eval.MarioAIlevelFromLatentVector(vector);
		MarioProcess marioProcess = new MarioProcess(new agents.runner.Agent());
		marioProcess.launchMario(new String[0], true); // true means there is a human player
		MarioResult result = marioProcess.simulateOneLevel(levelAI, true);
		MarioProcess replayProcess = new MarioProcess(new agents.replay.Agent(result.getAgentEvents()));
		replayProcess.launchMario(new String[0], true);
		replayProcess.simulateOneLevel(levelAI, true);



//		String content = "";
//		try {
//			content = new String(Files.readAllBytes(Paths.get("gson.txt")));
//		} catch (IOException e) {
//		}
//		Gson gson = new Gson();
//		ArrayList<MarioAgentEvent> data = gson.fromJson(content, new TypeToken<ArrayList<MarioAgentEvent>>() {}.getType());
//		MarioProcess replayProcess = new MarioProcess(new agents.replay.Agent(data));
//		replayProcess.launchMario(new String[0], true);
//		replayProcess.simulateOneLevel(levelAI, true);
