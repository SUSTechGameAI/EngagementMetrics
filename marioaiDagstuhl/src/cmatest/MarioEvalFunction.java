...
public class MarioEvalFunction implements IObjectiveFunction {
	...
	private MarioProcess marioProcessWeek;
	private MarioResult result;
	private MarioResult resultWeek;
	...
	public MarioEvalFunction() throws IOException {
		...
		ganProcess.start();
		// set up mario game
		marioProcess = new MarioProcess(new agents.runner.Agent());
		marioProcess.start();
		marioProcessWeek = new MarioProcess(new agents.runnerWeek.Agent());
		marioProcessWeek.start();
	...
	/**
	 * Takes a json String representing several levels
	 * and returns an array of all of those Mario levels used in Mario AI Framework.
	 * In order to convert a single level, it needs to be put into
	 * a json array by adding extra square brackets [ ] around it.
	 * @param json Json String representation of multiple Mario levels
	 * @return Array of those levels
	 */
	public static String[] marioAILevelsFromJson(String json) {
		List<List<List<Integer>>> allLevels = JsonReader.JsonToInt(json);
		String[] result = new String[allLevels.size()];
		int index = 0;
		for(List<List<Integer>> listRepresentation : allLevels) {
			result[index++] = LevelParser.createMarioAILevelJson(listRepresentation);
		}
		return result;
	}
	 ...
	 * @throws IOException Problems communicating with Python GAN process
	 */
	public String MarioAIlevelFromLatentVector(double[] x) throws IOException {
		x = mapArrayToOne(x);
		// Interpret x to a level
		// Brackets required since generator.py expects of list of multiple levels, though only one is being sent here
		ganProcess.commSend("[" + Arrays.toString(x) + "]");
		String levelString = ganProcess.commRecv(); // Response to command just sent
		String[] levels = marioAILevelsFromJson("[" +levelString + "]"); // Really only one level in this array
...
//			EvaluationInfo info = this.marioProcess.simulateOneLevel(level);
			String level;
			level = MarioAIlevelFromLatentVector(x);

			result = this.marioProcess.simulateOneLevel(level, false, 100);
			resultWeek = this.marioProcessWeek.simulateOneLevel(level, false, 100);
			...
			if(result.getCompletionPercentage() == 1 && resultWeek.getCompletionPercentage() < 1) { // Did not beat level
				...
				return  resultWeek.getCompletionPercentage() - result.getCompletionPercentage();
			}
	...
	public double valueOfWithSave(double[] x, String filename) {
		try {
//			Level level = levelFromLatentVector(x);
//			// Do a simulation
//			EvaluationInfo info = this.marioProcess.simulateOneLevel(level);
			...
			CMAMarioSolver.save_load_text(result, filename);
	...
	public double[] valueOfSeparate() {

		return new double[] {result.getCompletionPercentage(), resultWeek.getCompletionPercentage()};
