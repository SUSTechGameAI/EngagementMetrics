package competition.cig.killer.astar;

import ch.idsia.mario.engine.sprites.Mario;
import ch.idsia.ai.agents.Behavior;
import competition.cig.killer.astar.level.Level;
...
public class AStarSimulator 
{
	...
	private class SearchNode
	{
		...
		public int getKilled() {
			if (this.sceneSnapshot == null) return 0;

			return this.sceneSnapshot.enemiesKilled + this.sceneSnapshot.enemiesJumpedOn;
		}
		...
		public SearchNode(boolean[] action, int repetitions, SearchNode parent)
		{
	    	...
	    	if (parent != null)
	    	{
				...
				try {
					this.sceneSnapshot = (LevelScene) parentPos.sceneSnapshot.clone();
    ...
    private Behavior search(long startTime)
    {
    	...
    	behavior.setTotalStates(totalStates);
    	behavior.setDeathStates(deathStates);
    	return behavior;
    }
    ...
    private SearchNode pickBestPos(ArrayList<SearchNode> posPool)
    {
    	...
    	for (SearchNode current: posPool)
    	{
    		...
    		float currentCost = -10000 * current.getKilled();
	...
	public boolean[] optimise(Behavior behavior)
	{
		// do stuff
		...
		Behavior currentBehavior = search(startTime);
		behavior.addTotalStates(currentBehavior.getTotalStates());
		behavior.addDeathStates(currentBehavior.getDeathStates());
