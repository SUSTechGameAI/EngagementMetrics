package competition.cig.killer;

import ch.idsia.ai.agents.Behavior;
import ch.idsia.ai.agents.AgentWithBehavior;
import ch.idsia.mario.environments.Environment;
import competition.cig.killer.astar.AStarSimulator;
import competition.cig.killer.astar.sprites.Mario;

public class AStarAgent extends AgentWithBehavior
{
    protected boolean[] action = new boolean[Environment.numberOfButtons];
    protected String name = "Killer_AStarAgent";
	...
	int errCount = 0;
	AStarAgent errAgent;
	private Behavior agentBehavior = new Behavior();
    public void reset()
    {
    ...
    public boolean[] getAction(Environment observation)
    {
		...
		action = sim.optimise(agentBehavior);
